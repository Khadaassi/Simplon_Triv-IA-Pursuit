...
goat = pygame.image.load("media/the_goat.webp")
...
def draw_player(position):
    ...
    if isinstance(position, tuple):  # Si la position est une coordonnée
        x, y = position
    else:  # Sinon, calculer comme d'habitude
...
def check_answer(selected_index):
    ...
    total_questions+=1
    if isinstance(player_position, tuple):  # Si la position est une coordonnée
...
def draw_text_multiline(surface, text, color, rect, font, line_spacing=5):
    """
    Affiche un texte multi-ligne sur une surface en respectant une zone rectangulaire.
    
    :param surface: La surface sur laquelle dessiner le texte.
    :param text: Le texte à afficher.
    :param color: La couleur du texte.
    :param rect: Un tuple ou une liste définissant la zone (x, y, largeur, hauteur).
    :param font: La police utilisée pour dessiner le texte.
    :param line_spacing: L'espacement entre les lignes.
    """
    words = text.split(' ')
    lines = []
    current_line = []

    # Divise le texte en lignes adaptées à la largeur
    for word in words:
    ...
    # Affiche les lignes dans la zone spécifiée
    x, y, _, _ = rect
    for line in lines:
...
def draw_question():
    ...
    if current_question:
        ...
        # Définir une zone pour la question
        question_rect = pygame.Rect(screen_width / 2 + 200, screen_height / 2 - 180, 500, 100)
        
        # Dessiner la question avec retour à la ligne
        draw_text_multiline(screen, current_question, black, question_rect, smallfont)
        ...
        for i, choice in enumerate(current_choices):
            ...
            pygame.draw.rect(screen, bg_color, [x , y, 20, 20])
...
def all_cheeses_collected():
    """Vérifie si tous les camemberts ont été collectés."""
    return blue_cheese and green_cheese and pink_cheese and orange_cheese and yellow_cheese and purple_cheese

def move_to_center():
    """Déplace le joueur vers la case centrale pour la question finale."""
    global player_position
    player_position = center 
    return player_position

def load_mystery_question():
    """Charge une question mystérieuse."""
    global current_question, current_choices, current_answer
    pygame.mixer.music.load('media/wizards.mp3')
    pygame.mixer.music.play(loops = -1)
    
    current_question = f"Dans, Harry Potter, quel sort permet d'ouvrir des portes verrouillées tel un hack d'accès en cybersécurité?"
    current_choices = ["Lumos", "Alohomora", "Reparo", "Avada Kedavra"]
    current_answer = "Alohomora"

def draw_final_message():
    """Affiche un message de fin de jeu."""
    
    screen.blit(goat, (0, 0))
    message = middlefont.render("You are the GOAT!", True, white)
    screen.blit(message, (30,10))
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                mouse = pygame.mouse.get_pos()
                 ...
                 # Si toutes les camemberts sont collectés, gérer la question finale
                if all_cheeses_collected() and not game_over:
                    move_to_center()
                    load_mystery_question()
                    final_question = True

                # Gestion de la question finale
                if final_question:
                    for i in range(len(current_choices)):
                        ...
                        if x <= mouse[0] <= x + 20 and y <= mouse[1] <= y + 20:
                            check_answer(i)
                            if verification_done and validated_answers[current_choices.index(current_answer)]:
                                game_over = True
                                final_question = False
        ...
        if verification_done and not final_question:
            draw_button_reroll()
        if game_over:
            draw_final_message()
