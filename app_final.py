...
def final_question():
    """Pose la dernière question et détermine si le joueur gagne."""
    global current_question, current_choices, current_answer, validated_answers, verification_done, cheese_score

    # Vérifie si le joueur a tous les camemberts
    if cheese_score == 1:
        # Charger une question spéciale (dernière question)
        with open("media/questions_final.JSON", 'r', encoding='utf-8') as file:
            data_final = json.load(file)

            
        question_dict = random.choice(data_final)
        current_question = question_dict['question']
        current_choices = question_dict['choix']
        current_answer = question_dict['reponse']

        if verification_done:
            if any(validated_answers):
                # Si bonne réponse, victoire
                victory_message = bigfont.render("Victoire !", True, green)
                screen.blit(victory_message, (screen_width / 2 - 200, screen_height / 2))
                pygame.display.update()
                pygame.time.wait(3000)
                pygame.quit()  # Fin du jeu
            else:
                # Si mauvaise réponse, recommencer au prochain tour
                retry_message = smallfont.render("Mauvaise réponse ! Réessayez au prochain tour.", True, red)
                screen.blit(retry_message, (screen_width / 2 - 300, screen_height / 2 + 100))
                pygame.display.update()
                pygame.time.wait(3000)

        return True  # Dernière question affichée
    return False  # Pas encore prêt pour la question finale
...
def main():
    ...
    while running:
        ...
        # Dessine la question finale 
        if cheese_score == 1:
            # draw_player((screen_width / 2 - 450, screen_height / 2))
            screen.blit(star, (screen_width / 2 - 450, screen_height / 2))
            if final_question():
                continue  # Recommence la boucle si la dernière question est posée
        #initialize music
